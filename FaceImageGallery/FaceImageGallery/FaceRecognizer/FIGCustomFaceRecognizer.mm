...
@implementation FIGCustomFaceRecognizer
...
- (BOOL)trainModel
{
    ...
    if (sqlite3_prepare_v2(_db, selectSQL, -1, &statement, nil) == SQLITE_OK) {
        while (sqlite3_step(statement) == SQLITE_ROW) {
            ...
            cv::Mat faceData = [FIGOpenCVData dataToMat:imageData
...
- (void)learnFace:(cv::Rect)face ofPersonID:(int)personID fromImage:(cv::Mat&)image
{
    ...
    NSData *serialized = [FIGOpenCVData serializeCvMat:faceData];
